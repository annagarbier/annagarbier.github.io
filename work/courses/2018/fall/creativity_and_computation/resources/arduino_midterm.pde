 ...
 * MFADT, 2018
 *
 * 1) Reads Serial data from Arduino as a tab-separated string of
 *    three analog sensor values.
 * 2) Outputs "bubble noise(s)" when any of the three input
 *    values exceeds a threshold.
 *
 * Credits:
 * - "blop" noise comes from soundbible.com, with license to use.
 * - Method of reading Serial data into Processing as string found in
 *   several tutorials, e.g. https://www.youtube.com/watch?v=NhyB00J6PiM.
 */
...
void draw() {
  ...
  while (mySerial.available() > 0) {
    ...
    if (sensorString != null) {
      ...
      // todo: in future, use function for playing audio
      // todo: in future, set amplification based on pressure sensor value
...
void loop() {
  // For each of my analog sensor pins, write the value followed by a tab
